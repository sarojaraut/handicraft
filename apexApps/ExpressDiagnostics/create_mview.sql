CREATE MATERIALIZED VIEW MV_LOGGER_DATA
BUILD IMMEDIATE
REFRESH START WITH SYSDATE NEXT  (SYSDATE+5/(24*60))
WITH PRIMARY KEY
AS
WITH metric
 AS (SELECT 'Last Hour' name, 2 mid FROM DUAL
	 UNION
 ...
 log_data
 AS (  SELECT CASE
				 WHEN time_stamp >= SYSTIMESTAMP - INTERVAL '1' HOUR THEN 2
				 WHEN time_stamp >= TRUNC (SYSTIMESTAMP) THEN 4
				 ELSE 8
			  END
				 metric_id,
...
FROM metric LEFT JOIN log_data ld ON (metric.mid = ld.metric_id)
GROUP BY mid, name
/

COMMENT ON MATERIALIZED VIEW MV_LOGGER_DATA IS
'  NAME           : MV_LOGGER_DATA
  ALIAS          : MV_LOGGER_DATA
  ...
  -----------------------------------------------------------------------------
  1.0| 18/07/2016| S. Raut      | Creation   
'
/ 
COMMENT ON COLUMN MV_LOGGER_DATA.metric_id IS
'Metric ID can be 2, 4, 8.  Where 2=Last Hour, 4=Today and 8=This Week'
/
COMMENT ON COLUMN MV_LOGGER_DATA.error_count IS
'Number of error messages logged during the period period crresponding to the metric ID'
/
COMMENT ON COLUMN MV_LOGGER_DATA.warning_count IS
